...
# Update file paths to the respected input for the corpus creation here
greek_mono_file="/example_greek_mono.txt"
latin_mono_file="/example_latin_mono.txt"
greek_parallel_file="/example_greek_parallel.txt"
latin_parallel_file="/example_latin_parallel.txt"
...
def filter_parallel(source_list, target_list):
    ...
    return utils.deduplicate_list(sentence_pairs)  # set
...
def create_dataset(monolingual_source, monolingual_target, parallel_source_list, parallel_target_list, seed=42):
    ...
    print(
        f'{len(split_mono_source)} monolingual source sentences.\n{len(split_mono_target)} monolingual target sentences.')
    ...
    filter_para_source_list = [sent_pair[0] for sent_pair in filter_parallel_list]
    filter_para_target_list = [sent_pair[1] for sent_pair in filter_parallel_list]
...
def save_files(src, trg, main_path, data_list):
    ...
    with open(f'{main_path}/{src}-{trg}.train.{src}', 'w', encoding="utf-8") as f:
        f.write(data_list[0][0])
    with open(f'{main_path}/{src}-{trg}.train.{trg}', 'w', encoding="utf-8") as f:
        f.write(data_list[0][1])
    with open(f'{main_path}/{src}-{trg}.train.gold', 'w', encoding="utf-8") as f:
        f.write(data_list[0][2])

    with open(f'{main_path}/{src}-{trg}.test.{src}', 'w', encoding="utf-8") as f:
        f.write(data_list[1][0])
    with open(f'{main_path}/{src}-{trg}.test.{trg}', 'w', encoding="utf-8") as f:
        f.write(data_list[1][1])
    with open(f'{main_path}/{src}-{trg}.test.gold', 'w', encoding="utf-8") as f:
        f.write(data_list[1][2])

with open(latin_mono_file, encoding="utf-8") as f:
    latin_mono=f.readlines()
with open(greek_mono_file, encoding="utf-8") as f:
    greek_mono=f.readlines()
with open(latin_parallel_file, encoding="utf-8") as f:
    latin_para=f.readlines()
with open(greek_parallel_file, encoding="utf-8") as f:
    greek_para=f.readlines()

datalist=split_shuffle_create_corpus(greek_mono, latin_mono, greek_para, latin_para)

save_files("grc", "lat", output_dir, datalist)