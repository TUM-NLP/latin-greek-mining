...
def parse_args():
    ...
    parser.add_argument('-m', '--model_name', type=str, required=True, 
                        choices=['xlmr', 'glot500', 'pretrained', 'labse', 'sphilberta', 'ugarit', 'qwen'], help='Embedding model')
...
class EmbeddingLoader(object):
	def __init__(self, model: str="bert-base-multilingual-cased", device=torch.device('cuda'), layer: int=8):
		TR_Models = {
...
def to_labse_sentence_embeddings(path, sentence_list, model_name, start_i=0):
    if model_name=='labse':
        labse_model = SentenceTransformer('sentence-transformers/LaBSE', device="cuda")
    elif model_name=='sphilberta':
        labse_model = SentenceTransformer('bowphs/SPhilBerta', device="cuda")
    elif model_name=='qwen':
        labse_model = SentenceTransformer('Qwen/Qwen3-Embedding-0.6B', device="cuda")
    else:
...
def main():
    ...
    if model_name in ['xlmr', 'glot500', 'pretrained', 'ugarit']:
        to_xlmr_sentence_embeddings(args.output_file, split_file, model_name, start_i=0)
    elif model_name in ['labse', 'sphilberta', 'qwen']:
        to_labse_sentence_embeddings(args.output_file, split_file, model_name, start_i=0)
    return 0
